...
fetch('./json/mapa.json')
  ...
  .then(lugares => {
    ...
    function renderLista(filtro = '') {
      lista.innerHTML = '';
      lugares
        .filter(lugar => lugar.Lugar_ingles.toLowerCase().includes(filtro.toLowerCase()))
        .sort((a, b) => a.Número - b.Número)
        .forEach(lugar => {
          const coords = lugaresCoords[lugar.Número] || { x: 0, y: 0 };
          const li = document.createElement('li');
          li.className = 'list-group-item d-flex align-items-center gap-2';
          li.innerHTML = `<span class="direccion flex-grow-1" data-zoom="3" data-x="${coords.x}" data-y="${coords.y}"><i class="bi bi-geo-alt-fill me-2 text-primary"></i><strong>${lugar.Número}.</strong> ${lugar.Lugar_ingles}</span>`;
